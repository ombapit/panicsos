<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    public function edit_column($column, $content, $class, $match_replacement)
    {
      $this->edit_columns[$column][] = array('content' => $content,'class' => $class, 'replacement' => $this->explode(',', $match_replacement));
      return $this;
    }
    ...
    */
    ...
    private function first_order()
    {
        $this->ci->db->order_by($this->order,'desc');
    }
    ...
    private function exec_replace($custom_val, $row_data)
    {
      ...
      if(isset($custom_val['replacement']) && is_array($custom_val['replacement']))
      {
        foreach($custom_val['replacement'] as $key => $val)
        {
		  ...
		  //tambahan david
		  preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches);
		  if ($matches && !is_callable($matches[1])) {
			$matches[1] = array($custom_val['class'],$matches[1]);
		  }
		  //end david
		  if($matches && is_callable($matches[1])) {
			$func = $matches[1];
