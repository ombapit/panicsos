...
class Auth extends CI_Controller {
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			{
				// do we have a valid request?block by david
	...
	function create_user()
	{
		...
		if (validation_errors() != "" or $this->ion_auth->errors() == true) {
			$data['error'] =true;
			$errors = $this->form_validation->error_array();
			$i = 0;
			foreach ($errors as $key=>$val) {
				$data['field'][$i]['name'] = $key;
				$data['field'][$i]['message'] = $val;
				$i++;
			}
		}
	...
	{
		// bail if the email already exists
		$existing_email = $this->db->get_where('users', array('email' => $str))->row();
	...
	function edit_user()
	{
		...
		$this->form_validation->set_rules('email', $this->lang->line('create_user_validation_email_label'), 'required|valid_email|callback_email_check[' . $id . ']');
	...
	function edit_group()
	{
		...
		$this->form_validation->set_rules('description', $this->lang->line('edit_group_validation_desc_label'), 'xss_clean');
		...
		if (isset($_POST) && !empty($_POST))
		{
			if ($this->form_validation->run() === TRUE)
			{
				...
				$data['field'][0]['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
			}
		}
